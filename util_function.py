...
def busqueda_malla(datos, lista_cfg, n_prueba, metodo="sarima", paralelo=True):
    ...
    if paralelo:
        ...
        tareas = (delayed(calificar_modelo)(datos, n_prueba, metodo, cfg)
                  for cfg in lista_cfg)
        resultados = executor(tareas)
    else:
        resultados = [calificar_modelo(
            datos, n_prueba, metodo, cfg) for cfg in lista_cfg]
...
def calificar_modelo(datos, n_prueba, metodo, cfg, debug=False):
    ...
    llave = str(cfg)
    if debug:
        resultado, predicciones = validacion_al_frente(
            datos, n_prueba, metodo, cfg)
    else:
...
def encontrar_mejor_solucion(series, nombre, metodo="sarima", muestra=False):
    ...
    if muestra:
        ...
        while len(resultados) < 5:
            lista_cfg_azar = random.sample(lista_cfg, k=25)
            resultados += busqueda_malla(datos,
                                         lista_cfg_azar, n_prueba, metodo)
    else:
        # búsqueda en malla

        # Si se tiene capacidad de procesamiento paralelo, paralelo = True (default)
        paralelo = True
        resultados = busqueda_malla(
            datos, lista_cfg, n_prueba, metodo, paralelo)
